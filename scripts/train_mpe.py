...
def parse_args(args, parser):
    ...
    parser.add_argument("--use_safety_filter", type=lambda x:bool(strtobool(x)), 
                        default=False, help="Safety filter activated when true.")
    parser.add_argument("--dynamics_type", type=str, default=False, help="Agent's dynamics type: 'kinematic_vehicle', 'double_integrator'")
...
def main(args):
    ...
    # run dir
    save_dir = os.path.split(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))[0]
    run_dir = Path(save_dir + "/results") / all_args.scenario_name / all_args.experiment_name
    ...
    if all_args.share_policy:
        if all_args.env_name == "GraphMPE":
            # ## include masked runner if masked setting is on
            # if all_args.use_masking:
            #     from onpolicy.runner.shared.graph_mpe_runner_mask import GMPERunner as Runner
            # else:
