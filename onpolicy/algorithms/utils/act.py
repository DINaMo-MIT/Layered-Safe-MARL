...
class ACTLayer(nn.Module):
    ...
    def __init__(self, 
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            # print("Inputs DIM",inputs_dim)
            # print("action_space.n",action_space.n)
    ...
    def forward(self, 
        ...
        #     action_log_probs = action_logits.log_probs(actions)
            ...
            action_logits = self.action_out(x, available_actions)  ## remove available_actions to convert to Box from Discrete action space
            # print("ACTION LOGITS",action_logits,deterministic)
            actions = action_logits.mode() if deterministic else action_logits.sample() 
            action_log_probs = action_logits.log_probs(actions)
    ...
    def evaluate_actions(self, 
        ...
        if self.mixed_action:
            ...
            for action_out, act in zip(self.action_outs, action):
                action_logit = action_out(x)
                action_log_probs.append(action_logit.log_probs(act))
                # print("act active_masks",active_masks)
                if active_masks is not None:
                    # print("act active_masks",active_masks.shape)
                    # print("action_logit.entropy().shape",action_logit.entropy().shape)
                    dist_entropy.append((action_logit.entropy()*active_masks.squeeze(-1)).sum()/active_masks.sum())
            ...
            if torch.isnan(x).any():
                print("Tensor with NaN elements:")
                # Find NaN indices
                nan_indices = torch.isnan(x)
                x[nan_indices] = 0.0
            action_logits = self.action_out(x,available_actions) ## remove available_actions to convert to Box from Discrete action space 
            # print("action_logits",action_logits)
            # print("ACTION LOGITS entropy",action_logits.entropy())
            action_log_probs = action_logits.log_probs(action)
            # print("act active_masks",active_masks.shape, active_masks.T)
            if active_masks is not None:
                # print("act active_masks",active_masks.shape, active_masks.squeeze(-1).shape)
                # print("action_logit.entropy().shape",action_logits.entropy().shape)
