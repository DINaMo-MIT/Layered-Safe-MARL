...
class Viewer(object):
    ...
    def draw_line(self, start, end, **attrs):
        if 'linewidth' in attrs:
            ...
            geom = Line(start, end, linewidth=linewidth)
        else:
            geom = Line(start, end)
        _add_attrs(geom, attrs)
        self.add_onetime(geom)
...
def make_triangle_pointer(width, height, angle, filled=True):
    v1 = (0.5* width * math.sin(angle) - 0.33 * height * math.cos(angle), 
          - 0.5* width * math.cos(angle) - 0.33 * height * math.sin(angle))
    v2 = (0.67 * height * math.cos(angle), 0.67 * height * math.sin(angle))
    v3 = (- 0.5* width * math.sin(angle) - 0.33 * height * math.cos(angle), 
          0.5* width * math.cos(angle) - 0.33 * height * math.sin(angle))
    if filled: 
        return FilledPolygon([v1, v2, v3])
    else:
        return PolyLine([v1, v2, v3], True)
...
class Line(Geom):
    ...
    def render1(self):
        glLineWidth(self.linewidth.stroke)
        glBegin(GL_LINES)
