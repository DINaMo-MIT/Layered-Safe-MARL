...
class GR_Actor(nn.Module):
    ...
    def forward(self, 
        ...
        # print("node obs", node_obs.shape,node_obs[0])
        adj = check(adj).to(**self.tpdv)
        # print("adg", adj.shape, adj[0])
    ...
    def decode_actor_features(self, actor_features,
                                available_actions=None, 
                                deterministic=False):
        """
            Decode actor features to actions.
            actor_features: (torch.Tensor) 
                Actor features to decode.

            :return actions: (torch.Tensor) 
                Actions decoded from actor features.
        """
        if available_actions is not None:
            available_actions = check(available_actions).to(**self.tpdv)
    ...
    def evaluate_actions(self, 
        ...
        if (self.split_batch) and (obs.shape[0] > self.max_batch_size):
            ...
            nbd_features = self.gnn_base(node_obs, adj, agent_id)
            # print("nbd_features: ", nbd_features)
            actor_features = torch.cat([obs, nbd_features], dim=1)
...
class GR_Latent_Dynamics(nn.Module):
    # used arguments in args:
    ...
    def __init__(self, 
        ...
        super(GR_Latent_Dynamics, self).__init__()
        self.base = MLPLayer(input_dim=latent_dim,
                            hidden_size=256,
                            layer_N=2,
                            use_orthogonal=True,
                            use_ReLU=True,
    ...
    def forward(self, actor_features):
        return self.base(actor_features)