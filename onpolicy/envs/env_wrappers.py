...
def graphworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            if 'bool' in done.__class__.__name__:
                if done:
                    ob, ag_id, node_ob, adj, ep_info = env.reset(num_current_env_steps)
            else:
                if np.all(done):
                    ob, ag_id, node_ob, adj, ep_info = env.reset(num_current_env_steps)
            if ep_info:
            ...
            ob, ag_id, node_ob, adj, ep_info = env.reset(data)
            remote.send((ob, ag_id, node_ob, adj, ep_info))
        elif cmd == 'render':
            if data == "rgb_array":
                fr = env.render(mode=data)
                remote.send(fr)
...
class GraphDummyVecEnv(ShareVecEnv):
    ...
    def reset(self, num_current_episode: int = 0):
        ...
        obs, ag_id, node_obs, adj, ep_info = map(np.array, zip(*results))
        return obs, ag_id, node_obs, adj, ep_info
...
class GraphSubprocVecEnv(ShareVecEnv):
    ...
    def reset(self, num_current_episode: int = 0):
        ...
        results = [remote.recv() for remote in self.remotes]
        obs, ag_ids, node_obs, adj, info = zip(*results)
        return (np.stack(obs), np.stack(ag_ids), np.stack(node_obs), np.stack(adj), info)
